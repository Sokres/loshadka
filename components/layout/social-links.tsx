...
export function SocialLinks({
  ...
  const links = [
    vk ? { href: vk, label: "VK", Icon: VkIcon } : null,
    ...
    <div className={cn("flex items-center gap-3", className)}>
      {links.map(({ href, label, Icon }) => (
        <a
          ...
          aria-label={label}
          className={cn(
            "flex size-9 items-center justify-center rounded-lg transition hover:opacity-80",
            iconClassName ?? "text-primary hover:text-primary/80",
          )}
        >
